 ...
 * banker.cpp
 *
 * Student Name: Ahmad F Hassan
 * Student Number: 30055847
 *
...
public:
    ...
    bool isSafe (int pid, int * req, string & sequenceOrReason) {
        ...
        int * work = available;
        int * finish = new int [numProc];
        //Initialise finish vector
        for(int i = 0; i<numProc; i++){
          finish[i] = 0;  //0 indicates false (not finished)
        }
        //Process request
        for(int c=0; c<numResources; c++){
          if(req[c] > available[c]) {
            sequenceOrReason = "not enough resources available\n";
            return false;
          }else if(req[c] <= need[pid][c]) {
            work[c] -= req[c];
            allocation[pid][c] += req[c];
            need[pid][c] -= req[c];
          }else{
        ...
        while(j<numProc){
          ...
          for(int i = 0; i<numProc; i++){
            ...
            if(finish[i] == 0){
              //See what resource needs can be satisfied
              int needsSatisfied =0;
              for(int k=0; k < numResources; k++){
                //If instances of j needed by process i are less than or equal to
                //available
                if(need[i][k] <= work[k]){
                  needsSatisfied++;
                }
              ...
              if(needsSatisfied == numResources){
                //Grant resources to process
                ...
                //change process i to finished
                finish[i]=1;
                sequenceOrReason += " P";
                sequenceOrReason += n;
                sequenceOrReason += " ";
                //process meeting conditions was found
                found = true;
              }
            }
          }
        ...
        //Request does not result in an unsafe state
        return true;
    }
};
